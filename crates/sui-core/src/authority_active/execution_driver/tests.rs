...
async fn wait_for_certs(state: &Arc<AuthorityState>, certs: &Vec<VerifiedCertificate>) {
    if certs.is_empty() {
        if timeout(Duration::from_secs(30), state.next_ready_certificate())
            .await
            .is_err()
        {
            return;
    ...
    while !certs.is_empty() {
        match timeout(Duration::from_secs(5), state.next_ready_certificate()).await {
...
async fn pending_exec_notify_ready_certificates() {
    ...
    wait_for_certs(&authority_state, &certs).await;

    // Should have no certificate any more.
    wait_for_certs(&authority_state, &vec![]).await;
}
